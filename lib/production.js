...
const resolveLocations = async ({ rootPath, curPath, pkg, pkgCache }) => {
  // Production dependencies include both production and optional if found.
  const names = []
    .concat(Object.keys(pkg.dependencies || {}))
    .concat(Object.keys(pkg.optionalDependencies || {}));

  // TODO: After circular detection is fixed, then re-evaluate here.
  // In serial traverse the dependencies.
  // This is technically slower, but better for disk I/O contention and
  // potential cache hits.
  const locs = [];
  for (const name of names) {
    // Find current package.
    const found = await findPkg({ rootPath, curPath, name, pkgCache });
    if (!found) {
    ...
    const deps = await resolveLocations({ rootPath, curPath: found.loc, pkg: found.pkg, pkgCache });

    locs.push([path.relative(rootPath, found.loc)].concat(deps));
  }
 ...
 * @param {Object}  opts.pkgCache map of discovered package.json locations to avoid disk I/O
 * @returns {Promise<Array<String>>} list of relative paths to on-disk dependencies.
 */
const findProdInstalls = async ({ rootPath, curPath, pkgCache } = {}) => {
  ...
  return resolveLocations({ pkg, rootPath, curPath, pkgCache });
};
